...
def _parse_inventory_d(inventory_d: Path) -> list[dict]:
    """Parse all YAML files in inventory.d, failing if any file is unreadable."""
    entries = []
    for yaml_file in sorted(inventory_d.glob("*.yaml")):
        try:
            data = yaml.safe_load(yaml_file.read_text())
        except Exception as exc:
            raise RuntimeError(f"Failed to parse {yaml_file}: {exc}") from exc
...
def run_sync(db: Session) -> int:
    ...
    try:
        ...
        # Guarded on a non-empty parse so an unexpectedly empty inventory can't mass-clear.
        cleared = 0
        if entries:
